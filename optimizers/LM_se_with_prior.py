...
class LMOptimizerSE:
    def __init__(
        ...
        z,
        v,
        slk_bus,      # index of slack bus within the *angle* block
        nb,           # number of angles T
        delta_init,
        norm_H=None,
        sigma=0.1,
        K=500,
        ftol=1e-8,
        xtol=1e-8,
        m=None,       # full prior mean (includes slack)
        Q=None,       # full prior cov (includes slack)
        reg_scale=1.0,
        jitter=1e-10,
        prefix='-norm'
    ):
        ...
        if self._prior_enabled:
            ...
            if m.ndim != 1 or Q.ndim != 2 or Q.shape[0] != Q.shape[1] or Q.shape[0] != m.shape[0]:
                raise ValueError("m must be (n,), Q must be (n,n) over the FULL state (including slack).")

            n_full = m.shape[0]
            if n_full <= self.nb:
                raise ValueError("Full state must include angles (nb) and voltages; got n <= nb.")

            # Build a mask that drops ONLY the slack from the angle block [0:nb)
            slk_idx = int(self.slk_bus[0])
            if not (0 <= slk_idx < self.nb):
                raise ValueError("slk_bus index must be within the angle block [0, nb).")

            mask = np.ones(n_full, dtype=bool)
            mask[slk_idx] = False  # drop slack angle

            m_red = m[mask]
            Q_red = Q[np.ix_(mask, mask)]

            # Compute L such that L^T L = Q_red^{-1}
            Q_inv = np.linalg.inv(Q_red + 1e-10 * np.eye(len(Q_red)))
            # Q_inv = 0.5 * (Q_inv + Q_inv.T)  # symmetrize for numerical stability
            # Q_inv += jitter * np.eye(Q_inv.shape[0])
            # try:
            #     L = np.linalg.cholesky(Q_inv)
            # except np.linalg.LinAlgError:
            #     # Eigen fallback if Cholesky fails (e.g., nearly singular)
            #     w, U = np.linalg.eigh(Q_inv)
            #     w = np.clip(w, a_min=1e-15, a_max=None)
            #     L = (U * np.sqrt(w)) @ U.T  # symmetric sqrt
    ...
    def compute_J(self, x_full):
        T = x_full[:self.nb]
        V = x_full[self.nb:]
        _, J = self.h_ac.estimate(V, T)                 # measurement Jacobian wrt FULL x
        J = (- self.R @ J) / self.norm_H[:, np.newaxis] # weighted & normalized
        # Remove the slack column so J matches the reduced variable vector
        J = np.delete(J, self.slk_bus[0], axis=1)

        if self._prior_enabled:
            # Augment with prior rows: J_prior = L (already reduced shape)
    ...
    def compute_f(self, x_full):
        ...
        if self._prior_enabled:
            # Build reduced x by removing the slack angle component
    ...
    def compute_rho(self, x, D, step_size, alpha):
        # Uses augmented f and J (meas + prior)
        norm_f = np.linalg.norm(self.compute_f(x))
        norm_fp = np.linalg.norm(self.compute_f(x + np.insert(step_size, self.slk_bus[0], 0)))
        norm_Jp = np.linalg.norm(self.compute_J(x) @ step_size)
        norm_Dp = np.linalg.norm(D @ step_size)
    ...
    def optimize(self, x0):
        x = x0.copy()  # FULL state (including slack angle)
        J = self.compute_J(x)
        f = self.compute_f(x)
        D = self.compute_D(J)
        ...
        for k in tqdm(range(self.K), desc=f"Optimizing with LM{self.prefix}", leave=False, colour='green'):
            ...
            if rho > 1e-4 or np.linalg.norm(step_size, np.inf) <= self.xtol:
                x = x + np.insert(step_size, self.slk_bus[0], 0)  # reinsert slack=0 step
                J = self.compute_J(x)
                f = self.compute_f(x)
