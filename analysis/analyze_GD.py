...
from data_parser.data_generator import DataGenerator
from optimizers.LM_se_with_prior import LMOptimizerSE
...
def run_experiment(data, sys, branch, init_params, **kwargs):
    ...
    m_GST = np.r_[T0, V0]
    Q_GST = np.linalg.pinv(-np.block([[0.045 * np.imag(sys.Ybus), np.zeros((sys.nb, sys.nb))],
                  [np.zeros((sys.nb, sys.nb)), 10 * np.imag(sys.Ybus)]]))
    # Q = np.diag(np.r_[np.ones(sys.nb) * np.pi * init_params[0] / 3, np.ones(sys.nb) * .05 / 3] ** 2)
    data_generator = kwargs.get('data_generator', None)
    m = data_generator.m
    Q = data_generator.cov
    ...
    for i in range(100):
        ...
        # T0, V0 = init_start_point(sys, data, how='dc', dc_init=(branch, measurements, variance, meas_idx, agg_meas_idx))
        x0 = np.r_[T0, V0]
        ...
        reg_scale_GST = 1
        while np.abs(reg_scale_GST - prev_reg_scale_GST) > 1e-7:
